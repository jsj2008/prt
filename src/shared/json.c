  ...
  if (variant->type == JSON_VARIANT_STRING) {
    ...
    if (!ret->string)
      return -ENOMEM;
             ...
             variant->type == JSON_VARIANT_OBJECT) {
    size_t i;

    ret->objects = (JsonVariant *)calloc(sizeof(JsonVariant), variant->size);
    if (!ret->objects)
      return -ENOMEM;
...
static void _json_variant_unrefp(JsonVariant **p) { freec((void **)p); }
#define __cleanup_json_variant                                                 \
  __attribute__((__cleanup__(_json_variant_unrefp)))

static void _json_variant_array_unrefp(JsonVariant ***p) { freec((void **)p); }
#define __cleanup_json_variant_array                                           \
  __attribute__((__cleanup__(_json_variant_array_unrefp)))
  ...
  for (;;) {
    ...
    if (*c == '\\') {
      ...
      if (in_set(*c, '"', '\\', '/'))
        ...
        s = (char *)reallocarray(s, sizeof(*s), n + 4);
        ...
        if (!utf16_is_surrogate(x))
          ...
          n +=
              utf8_encode_unichar(s + n, utf16_surrogate_pair_to_unichar(x, y));
        }
      ...
      s = (char *)reallocarray(s, sizeof(*s), n + 2);
    ...
    s = (char *)reallocarray(s, sizeof(*s), n + len + 1);
  ...
  *p = c;
  ...
  if (is_double) {
    ret->real = ((negative ? -1.0 : 1.0) * (x + (y / shift))) *
                exp10f((exponent_negative ? -1.0 : 1.0) * exponent);
    return JSON_REAL;
  } else {
...
int json_tokenize(const char **p, char **ret_string,
  ...
  for (;;) {
    ...
    case STATE_VALUE:
      ...
      } else if (str_starts_with(c, "true")) {
        *ret_string = NULL;
      ...
      } else if (str_starts_with(c, "false")) {
        *ret_string = NULL;
      ...
      } else if (str_starts_with(c, "null")) {
        *ret_string = NULL;
        *ret_value = JSON_VALUE_NULL;
        *p = c + 4;
        *state = INT_TO_PTR(STATE_VALUE_POST);
...
static int json_scoped_parse(JsonVariant **tokens, size_t *i, size_t n,
                             JsonVariant *scope) {
  ...
  while ((var = *i < n ? tokens[(*i)++] : NULL) != NULL) {
    ...
    if (state == STATE_KEY) {
      ...
      __cleanup_json_variant JsonVariant *v = NULL;
      size_t toadd = arr ? 1 : 2;

      if (!json_is_value(var)) {
        int type = (var->value.integer == JSON_ARRAY_OPEN)
                       ? JSON_VARIANT_ARRAY
                       : JSON_VARIANT_OBJECT;
      ...
      items = (JsonVariant *)reallocarray(items, sizeof(*items), size + toadd);
...
static int json_parse_tokens(JsonVariant **tokens, size_t ntokens,
                             JsonVariant **rv) {
  size_t it = 0;
  int r;
  JsonVariant *e;
  __cleanup_json_variant JsonVariant *p = NULL;
...
static int json_tokens(const char *string, size_t size, JsonVariant ***tokens,
                       size_t *n) {
  __free_str char *buf = NULL;
  __cleanup_json_variant_array JsonVariant **items = NULL;
  union json_value v = {};
  ...
  p = buf;
  for (;;) {
    __cleanup_json_variant JsonVariant *var = NULL;
    __free_str char *rstr = NULL;
    ...
    items = (JsonVariant **)reallocarray(items, sizeof(*items), s + 2);
    if (!items) {
      return -ENOMEM;
    }
...
int json_parse(const char *string, size_t size, JsonVariant **rv) {
  __cleanup_json_variant_array JsonVariant **s = NULL;
  JsonVariant *v = NULL;
  size_t n = 0;
  int r;
